...
Most WorkOS setup failures are one of two things: a redirect URI that does not match exactly, or a session that is never refreshed. Get both right first and the rest follows. Use `11ai-workos-environment` to inspect what already exists before writing anything.
