---
name: 11ai-operator-bash-processes
...
- Report the process identified with its full command line, the signal sent, whether it exited on `SIGTERM` or needed `SIGKILL`, the exit status or `PIPESTATUS`, and anything left running. If a script hangs rather than a process misbehaving, hand off to `11ai-operator-bash-troubleshooting`.
