---
name: 11ai-operator-nodejs-api-setup
...
If any server code already exists, this is not a scaffold. Use `11ai-operator-nodejs-api-environment` to inspect it and change only what is broken; adding a second framework or a second configuration loader alongside a working one is the failure this step prevents.
...
5. **An error boundary** that maps thrown errors to status codes and never returns a stack trace to a client. `11ai-operator-nodejs-api-errors` owns the detail.
