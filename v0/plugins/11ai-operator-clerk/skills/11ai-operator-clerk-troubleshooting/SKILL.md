---
name: 11ai-operator-clerk-troubleshooting
...
- **Sign-in works but nothing is protected** — authorization is done with `SignedIn` or `Protect` in components. Those decide what renders; the route still serves. Confirm with `curl` and add a server check.
...
Hand off when the cause is elsewhere: `11ai-operator-clerk-environment` for instance and wiring questions, `11ai-operator-clerk-setup` if the provider or middleware is missing, `11ai-operator-clerk-authentication` for flows and redirects, `11ai-operator-clerk-sessions` for protection and permissions, `11ai-operator-clerk-users` for records and metadata, `11ai-operator-clerk-organizations` for tenant scoping, and `11ai-operator-clerk-webhooks` for event delivery.
