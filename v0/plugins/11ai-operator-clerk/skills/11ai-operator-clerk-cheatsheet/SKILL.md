---
name: 11ai-operator-clerk-cheatsheet
...
Lead with the call. Add one line on what it changes, whether it is server-only, and which instance it acts on. Name the operation skill when the task goes beyond a lookup: sign-in flows to `11ai-operator-clerk-authentication`, route protection to `11ai-operator-clerk-sessions`, user records to `11ai-operator-clerk-users`, organizations to `11ai-operator-clerk-organizations`, events to `11ai-operator-clerk-webhooks`, and failures to `11ai-operator-clerk-troubleshooting`.
