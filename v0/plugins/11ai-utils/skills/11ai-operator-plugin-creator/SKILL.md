...
description: "Scaffold a new 11ai operator plugin for a tool, CLI, service, or SDK so it matches the existing operator plugins exactly, covering the skill archetypes, canonical frontmatter, safety contract, setup and integrations reference docs, Codex metadata, plugin manifests, and catalog wiring. Use when asked to create an operator plugin, add skills for a new tool, extend an existing operator plugin with a missing archetype, or check that an operator plugin follows the house pattern."
---
# 11ai operator plugin creator

Build a plugin named `11ai-operator-TOOL` that a reader cannot tell apart from the ones already in this repository. The shape is fixed, not a preference: `v0/scripts/validate-skills.mjs` rejects off-pattern frontmatter, missing Codex metadata, broken links, and catalog counts that do not add up. Read [references/plugin-blueprint.md](references/plugin-blueprint.md) before writing any file.
...
```text
v0/plugins/11ai-operator-TOOL/
  .claude-plugin/plugin.json
  .codex-plugin/plugin.json
  README.md
  skills/11ai-operator-TOOL-AREA/
    SKILL.md
    agents/openai.yaml
    references/setup.md
```
