---
name: 11ai-audit-security-dependencies
...
# 11ai Audit Security Dependencies
...
- Prefer lockfile-only audits. Inspect installed files as data, not by importing
  them. Keep raw audit output in the namespaced temporary evidence directory
  defined below and avoid exposing tokens or private registry URLs in the report.
...
Create the report run directory as soon as the project root is established:

```bash
python3 scripts/prepare_report_output.py /path/to/project
```

Use the returned `run_directory`, `artifact_prefix`, `markdown_report`, and
`html_report` paths throughout the audit. The helper creates the two directories
and reserves unique report paths; completing the audit produces this structure:

```text
<project-root>/
└── 11ai-audit-security-dependencies-reports/
    └── 11ai-audit-security-dependencies-reports-YYYYMMDDTHHMMSSZ/
        ├── 11ai-audit-security-dependencies-reports-YYYYMMDDTHHMMSSZ.md
        ├── 11ai-audit-security-dependencies-reports-YYYYMMDDTHHMMSSZ.html
        └── <other sanitized outputs>
```

The helper uses the current UTC time. If a same-second directory already
exists, it appends `-2`, `-3`, and so on to the run directory and every report
basename. Never reuse or overwrite a previous run directory.
...
python3 scripts/scan_dependencies.py /path/to/project --format json --output /path/to/temporary-working-directory/11ai-audit-security-dependencies-inventory.json
```

Keep unsanitized inventory and raw audit responses in a unique ignored temporary
working directory outside the project. Do not treat temporary working files as
deliverable outputs, and delete them when the retained reports have been
verified. Sanitize any machine-readable artifact before copying it into the run
directory. Derive retained artifact names from `artifact_prefix`:

```text
<artifact-prefix>-inventory.json
<artifact-prefix>-candidate-ledger.json
<artifact-prefix>-audit-<ecosystem>-<scope>.json
```

Write every retained output under `run_directory` and prefix its basename with
the run directory's basename. Tool-owned caches and deleted temporary working
files are exempt.
...
Write two equivalent UTF-8 reports to the `markdown_report` and `html_report`
paths returned in step 1:

```text
11ai-audit-security-dependencies-reports/11ai-audit-security-dependencies-reports-YYYYMMDDTHHMMSSZ/11ai-audit-security-dependencies-reports-YYYYMMDDTHHMMSSZ.md
11ai-audit-security-dependencies-reports/11ai-audit-security-dependencies-reports-YYYYMMDDTHHMMSSZ/11ai-audit-security-dependencies-reports-YYYYMMDDTHHMMSSZ.html
```

Use the exact collision-adjusted basename returned by the helper when it ends in
`-2`, `-3`, or another suffix. Never write either report directly into the
project root and never overwrite a previous report. Do not include raw audit
responses, secrets, tokens, or private registry credentials. Include sanitized
summaries and source references only.

Keep the Markdown and HTML reports semantically equivalent: include the same
verdict, findings, evidence, remediation, limitations, timestamps, and links in
both. Make the HTML a standalone HTML5 document with UTF-8 metadata, a useful
`title`, accessible heading and table structure, and restrained inline CSS.
Escape all untrusted values before inserting them into HTML. Do not include
JavaScript, remote assets, tracking, or executable content. Add reciprocal
relative links between the Markdown and HTML reports.
...
```markdown
_Security audit by [11ai-audit-security-dependencies](https://ai.rj11.io/skills/11ai-audit-security-dependencies)._
```

End the HTML report with the equivalent linked signature:

```html
<p><em>Security audit by <a href="https://ai.rj11.io/skills/11ai-audit-security-dependencies">11ai-audit-security-dependencies</a>.</em></p>
```
...
Before finishing, verify that both reports exist inside the intended run
directory, are non-empty, contain the same audit timestamp and coverage
reconciliation, use the required linked signatures, and do not expose
credentials. Verify that the HTML parses as HTML5 and contains no `script`
element or remote asset. Return both absolute paths as clickable file links in
the final response.
