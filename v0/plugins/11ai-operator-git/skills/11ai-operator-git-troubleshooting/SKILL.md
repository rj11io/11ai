---
name: 11ai-operator-git-troubleshooting
...
Conclude with the failing boundary, evidence, root cause or remaining uncertainty, exact fix applied or proposed, and verification result. Hand off to `11ai-operator-git-recovery` for undo/recovery decisions and `11ai-operator-git-sync` for explicit synchronization choices.
