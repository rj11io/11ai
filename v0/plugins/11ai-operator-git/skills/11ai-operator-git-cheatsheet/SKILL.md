---
name: 11ai-operator-git-cheatsheet
...
For any state-changing command, first show the target path, branch, or commit and finish by checking `git status --short --branch`. For errors or surprising state, use `11ai-operator-git-troubleshooting` instead of guessing from one command.
