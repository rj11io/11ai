...
If nothing is staged, stop and ask whether to use `11ai-operator-git-stage` and which paths or hunks should be included. If unrelated changes are staged, stop rather than absorbing them. Check for secrets, generated files, and accidental environment changes. Use `11ai-operator-git-conventional-commits` to choose and validate a Conventional Commits 1.0.0 message, including any compatible repository-specific types, scopes, or wording rules.
...
Respect hooks and report hook failures. Do not add `--no-verify`, `--amend`, `--allow-empty`, or a second commit to work around a failure without explicit direction. Do not push; use `11ai-operator-git-sync` for that as a separate operation.
