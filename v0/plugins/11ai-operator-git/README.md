...
| Skill | Use it for |
| --- | --- |
| [`11ai-operator-git-main-operator`](./skills/11ai-operator-git-main-operator/SKILL.md) | Working directly on the main branch: sync a clean tree, do the task, quality-check, and report — committing or pushing only when explicitly asked |
| [`11ai-operator-git-branch-operator`](./skills/11ai-operator-git-branch-operator/SKILL.md) | Landing work through a branch and pull request: branch from a clean tree, do the task, open a detailed PR, run two review subagents, address their comments — merging or closing only when explicitly asked |
...
| [`11ai-operator-git-conventional-commits`](./skills/11ai-operator-git-conventional-commits/SKILL.md) | Choosing and validating a Conventional Commits 1.0.0 message whenever an operator needs to commit its work |
...
The operators wrap a task; they do not define one. Name the task (or the task skill) in the same request, for example: "Use 11ai-operator-git-branch-operator with 11ai-super-ux to improve the dashboard's accessibility." The focused skills can be used alone for a single Git operation, for example: "Use 11ai-operator-git-status to tell me what is ahead of origin/main."
