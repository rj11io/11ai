...
This is the one SSH skill that changes the machine and can change a server's authorized keys. Confirm which host, which user, and how the public key will get there before creating anything. `11ai-ssh-environment` inspects what already exists; run it first so an existing key is reused rather than replaced.
...
- Report the client version, the key path and fingerprint, whether the agent holds it, the permissions set, any host entry added, where the public key was authorized, and the verification exit code. Hand off to `11ai-ssh-troubleshooting` if the connection still fails.
