...
If the client or a first key is missing, hand off to `11ai-ssh-setup`. If the effective configuration is the cause, hand off to `11ai-ssh-config`. If a key must be rotated or authorized, hand off to `11ai-ssh-keys`.
