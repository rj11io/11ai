...
Lead with the syntax. Add one line on the trap that bites — unquoted expansion, missing `pipefail`, `[ ]` instead of `[[ ]]`, a loop over `ls`. Name the operation skill when the task goes beyond a lookup: writing a script to `11ai-bash-scripting`, text work to `11ai-bash-text-processing`, file operations to `11ai-bash-files`, process control to `11ai-bash-processes`, and a failing script to `11ai-bash-troubleshooting`.
