...
- Report the files written, the strict-mode options set, the exit codes the script uses and what each means, the shellcheck result, and which failure paths were tested. Hand a still-failing script to `11ai-bash-troubleshooting`, and lint or test setup to `11ai-bash-testing`.
