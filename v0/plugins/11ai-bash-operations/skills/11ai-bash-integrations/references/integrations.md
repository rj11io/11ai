...
Hooks in `.git/hooks` are not committed, so a hook manager is what makes them shared. See `11ai-git-integrations` for husky and lefthook setup; the script side is what matters here.
