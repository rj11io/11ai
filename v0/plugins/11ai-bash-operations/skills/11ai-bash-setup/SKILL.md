...
Two things get changed here, and both are shared state: the machine's shells, and the user's startup files. A wrong edit to a startup file can leave a shell that will not open, so read the current state first and append rather than rewrite. `11ai-bash-environment` inspects without touching; run it first.
