...
This is the one Docker skill that changes the machine, so establish what is already there before installing anything. `11ai-docker-environment` inspects without touching; use it first and let its findings decide what this skill needs to do.
...
- Report the client and server versions, the active context, Compose and buildx availability, what was changed, and the verification output. If the daemon still does not answer, hand off to `11ai-docker-troubleshooting`.
