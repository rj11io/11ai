---
name: 11ai-operator-workos-troubleshooting
...
- **A connection exists but nobody can sign in through it** — its state is not active, so configuration on the identity provider side is unfinished. That is a customer action; hand off to `11ai-operator-workos-sso`.
...
- **A user removed from the directory still has access** — deactivation updated a row but did not end their sessions, or the event was missed and no reconciliation pass runs. Hand off to `11ai-operator-workos-directory-sync`.
...
Hand off when the cause is elsewhere: `11ai-operator-workos-environment` for key and configuration questions, `11ai-operator-workos-setup` if values or routes are missing, `11ai-operator-workos-authkit` for session and route protection, `11ai-operator-workos-sso` for connections, `11ai-operator-workos-directory-sync` for provisioning, and `11ai-operator-workos-webhooks` for event delivery.
