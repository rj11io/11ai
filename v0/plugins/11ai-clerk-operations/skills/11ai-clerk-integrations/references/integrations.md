...
See `11ai-clerk-setup` for the provider and middleware. The server helpers are `auth()`, `currentUser()`, and `clerkClient()`.
...
Store the link in both directions: the customer id on your tenant row, and the organization id in the customer's metadata. That is what makes reconciliation possible later. See `11ai-stripe-customers`.
