---
name: 11ai-convex-troubleshooting
...
- **A schema push rejected** — existing documents do not match the new definition. It can pass in development and fail on production because production has different data. Widen, backfill, then narrow; see `11ai-convex-schema`.
...
- **Open browser tabs breaking right after a deploy** — the deploy removed or narrowed a function those clients still call. Clients do not reload. Add first, ship, remove later; see `11ai-convex-deployments`.
...
Hand off when the cause is elsewhere: `11ai-convex-environment` for deployment and variable questions, `11ai-convex-setup` if the project is not linked, `11ai-convex-schema` for rejected pushes and indexes, `11ai-convex-functions` for limits and authorization, `11ai-convex-actions` for external calls and retries, and `11ai-convex-deployments` for anything that started at a deploy.
