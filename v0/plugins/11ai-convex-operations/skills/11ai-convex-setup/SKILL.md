...
Use `11ai-convex-environment` for the full inspection. If a `convex/` directory already exists with functions, this is not a fresh setup — add only what is missing rather than initializing over someone's work.
...
- Do not run `npx convex deploy` during setup. That pushes to production and belongs in `11ai-convex-deployments`.
