...
Lead with the command or call. Add one line on what it changes, whether it targets development or production, and whether it must be internal. Name the operation skill when the task goes beyond a lookup: schema to `11ai-convex-schema`, functions to `11ai-convex-functions`, external calls to `11ai-convex-actions`, deploys to `11ai-convex-deployments`, and failures to `11ai-convex-troubleshooting`.
