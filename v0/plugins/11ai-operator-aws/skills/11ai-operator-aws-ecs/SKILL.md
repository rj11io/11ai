...
After an approved change, verify deployment stability, task health, service events, target health, logs, and the original endpoint or job behavior. If the failure is primarily an image problem, hand off to `11ai-operator-aws-ecr`; if it is primarily logs or metrics, use `11ai-operator-aws-cloudwatch`.
