---
name: 11ai-operator-stripe-checkout
...
Also handle `customer.subscription.updated` and `customer.subscription.deleted`, or a later cancellation or payment failure leaves access granted forever. `checkout.session.completed` is the start of the story, not all of it. See `11ai-operator-stripe-subscriptions`.
