---
name: 11ai-operator-ssh-cheatsheet
...
Lead with the command. Add one line on what it changes and any flag that makes it destructive or externally visible. Name the operation skill when the task goes beyond a lookup: keys to `11ai-operator-ssh-keys`, host entries to `11ai-operator-ssh-config`, forwarding to `11ai-operator-ssh-tunnels`, transfers to `11ai-operator-ssh-file-transfer`, failures to `11ai-operator-ssh-troubleshooting`.
