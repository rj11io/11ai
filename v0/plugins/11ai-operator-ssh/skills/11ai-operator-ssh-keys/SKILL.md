...
Report the key path, type, comment, and fingerprint; where the public key was authorized; whether the agent holds it; what was removed and from which hosts; and the verification exit code. Never include private key material. If a key that should work does not, check file permissions first and then hand off to `11ai-operator-ssh-troubleshooting`.
