---
name: 11ai-operator-ssh-sessions
...
Report the effective host and user, the exact command run, the exit status, the relevant output with secrets redacted, and what changed on the remote host. If the connection itself failed with `255`, hand off to `11ai-operator-ssh-troubleshooting` rather than retrying the command.
