...
1. Inspect what already exists before adding anything: the pipeline's current authentication step, the registry and repository names, the infrastructure tool and where its state lives, and how the application reads configuration today. Use `11ai-aws-environment` to confirm which account you are looking at.
