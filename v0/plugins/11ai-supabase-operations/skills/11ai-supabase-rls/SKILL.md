...
Policies are schema. Put every statement in a migration file rather than only in Studio, or it exists in one environment and disappears on the next reset. Hand the file to `11ai-supabase-migrations`.
