...
Every schema object created here belongs in a migration file, not only in a live statement. A function created directly in Studio exists in one environment and vanishes on the next `db reset`. Hand it to `11ai-supabase-migrations`.
...
For a slow query, read the plan before adding anything. A sequential scan on a large table usually wants an index; add it through `11ai-supabase-migrations` so it exists in every environment rather than only this one.
