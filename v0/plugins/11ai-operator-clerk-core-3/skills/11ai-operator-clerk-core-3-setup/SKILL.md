---
name: 11ai-operator-clerk-core-3-setup
...
Use `11ai-operator-clerk-core-3-environment` for the full inspection. If a provider and middleware already exist, this is not a fresh setup — change only what is missing.
...
Core 3's `Show` component controls what renders. It is not access control — anything it hides is still reachable by requesting the route directly, and any data fetched above it is still fetched. Every route that returns private data must check on the server.
...
- Do not rely on `Show` for access control. Check on the server.
