---
name: 11ai-operator-clerk-core-3-environment
...
Compare the two counts. The legacy names are included to flag Core 2 code that must migrate to `Show`. Components that hide interface elements are not access control; if the first number is large and the second is near zero, authorization is being done in the browser only and every route is reachable directly.
...
- **Sign-in working and protection not** — authorization is done with `Show` (or removed Core 2 components) in the client only. Those hide elements; the route still serves.
