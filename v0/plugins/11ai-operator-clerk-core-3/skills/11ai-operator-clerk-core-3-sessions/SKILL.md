---
name: 11ai-operator-clerk-core-3-sessions
...
The distinction that matters: Core 3's `Show` decides what renders, and `auth()` decides what a request is allowed to do. Hiding a button changes nothing about whether the route serves data. Every route returning private data must check on the server.
...
Compare the last two counts. `SignedIn`, `SignedOut`, and `Protect` are included only to detect removed Core 2 patterns. A large number of component guards with almost no server checks means authorization lives in the browser, and every route is reachable directly with `curl`.
