---
name: 11ai-operator-aws-cli-v2-troubleshooting
...
If a fix is approved, make one bounded change, record the exact target, then re-run the original check and verify the user-visible objective. If the issue is primarily identity, use `11ai-operator-aws-cli-v2-iam`; if it is a service-specific workflow, hand off to that skill.
