...
- confirm both formats contain root/sub-agent counts and relationship detail, plus `Cost by model by effort` as a level-two sibling immediately after `Cost by model`, wall time, active time, both hourly cost rates where calculable, totals, pricing coverage, limitations, and the exact linked signature;
