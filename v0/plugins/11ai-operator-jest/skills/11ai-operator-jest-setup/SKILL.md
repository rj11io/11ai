...
Keep the continuous integration flags out of `test` itself so `npm test` stays useful locally; `11ai-operator-jest-ci` owns the pipeline command.
...
- Do not add `--forceExit` or `--detectOpenHandles` to the default script to make a hang go away. That is `11ai-operator-jest-troubleshooting` work.
- Do not enable coverage thresholds during setup. Get the suite running first.
- Report the transform chosen and why, the environment, files created, dependencies added, and the output of `--listTests`. If tests run but fail on module resolution or syntax, hand off to `11ai-operator-jest-troubleshooting`.
