...
try {
  ...
  assert.match(html, /<h2>All time<\/h2>/)
  assert.equal((html.match(/<h3>Totals<\/h3>/g) ?? []).length, 4)
  assert.equal((html.match(/<h3>Cost by harness<\/h3>/g) ?? []).length, 4)
  assert.match(html, /<h2>Year to date<\/h2>/)
  assert.match(html, /<h2>Month to date<\/h2>/)
  assert.match(html, /<h2>Past 7 days<\/h2>/)
  assert.match(html, /<table>/)
  assert.match(html, /codex-session\/[^/]+\/sessions\/recent\.jsonl/)
  assert.ok(html.indexOf("<h2>All time</h2>") < html.indexOf("<h2>Year to date</h2>"))
  assert.ok(html.indexOf("<h2>Year to date</h2>") < html.indexOf("<h2>Month to date</h2>"))
  assert.ok(html.indexOf("<h2>Month to date</h2>") < html.indexOf("<h2>Past 7 days</h2>"))
  ...
  assert.equal(dirname(defaultSummary.outputDirectory), fakeDesktop)
  assert.match(basename(defaultSummary.outputDirectory), /^11ai-global-llm-cost-\d{4}-\d{2}-\d{2}T/)
  ...
  assert.equal(basename(defaultSummary.markdownReport), `${basename(defaultSummary.outputDirectory)}.md`)
  assert.equal(basename(defaultSummary.htmlReport), `${basename(defaultSummary.outputDirectory)}.html`)
} finally {
